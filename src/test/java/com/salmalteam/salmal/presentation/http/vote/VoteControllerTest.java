...
class VoteControllerTest extends PresentationTest {
	...
	class 투표_삭제_테스트 {
		...
		void 투표_삭제_성공() throws Exception {
			...
			final ResultActions resultActions = mockMvc.perform(
					RestDocumentationRequestBuilders.delete(BASE_URL + URL, voteId)
						.header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN)
						.characterEncoding(StandardCharsets.UTF_8)
						.contentType(MediaType.APPLICATION_JSON))
	...
	class 투표_평가_테스트 {
		...
		void 투표_평가_성공() throws Exception {
			...
			resultActions.andDo(restDocs.document(
				...
				requestFields(
					fieldWithPath("voteEvaluationType").type(JsonFieldType.STRING)
						.description("투표 평가 타입 ( LIKE, DISLIKE )")
				)
			));
	...
	class 투표_조회 {
		...
		void 투표_조회_성공() throws Exception {
			...
			final VoteResponse voteResponse = new VoteResponse(voteId, memberId, "imageUrl", "닉네임", "member-imageUrl",
				3, 10, 10, 20, BigDecimal.valueOf(0.5), BigDecimal.valueOf(0.5), LocalDateTime.now(), true, "LIKE");
			...
			// when
			final ResultActions resultActions = mockMvc.perform(
					RestDocumentationRequestBuilders.get(BASE_URL + URL, voteId)
						.header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN)
						.characterEncoding(StandardCharsets.UTF_8)
						.contentType(MediaType.APPLICATION_JSON))
			...
			resultActions.andDo(restDocs.document(
				...
				responseFields(
					...
					fieldWithPath("status").type(JsonFieldType.STRING)
						.description("해당 투표에 대한 나의 상태 (NONE, LIKE, DISLIKE)")
				)
			));
		}
	...
	class 투표_목록_조회 {
		...
		void 투표_조회_성공() throws Exception {
			...
			final VoteResponse voteResponse1 = new VoteResponse(2L, 23L, imageURL, "사과", imageURL, 23, 50, 50, 100,
				BigDecimal.valueOf(0.5), BigDecimal.valueOf(0.5), LocalDateTime.now(), true, "NONE");
			final VoteResponse voteResponse2 = new VoteResponse(3L, 2L, imageURL, "포도", imageURL, 2, 20, 10, 30,
				BigDecimal.valueOf(0.67), BigDecimal.valueOf(0.33), LocalDateTime.now(), false, "LIKE");
			final VoteResponse voteResponse3 = new VoteResponse(1L, 100L, imageURL, "옥수수", imageURL, 10, 10, 10, 20,
				BigDecimal.valueOf(0.5), BigDecimal.valueOf(0.5), LocalDateTime.now(), true, "DISLIKE");

			final VotePageResponse votePageResponse = VotePageResponse.of(true,
				List.of(voteResponse1, voteResponse2, voteResponse3));
	...
	class 투표_댓글_목록_조회_테스트 {
		...
		void 댓글_목록_조회_성공() throws Exception {
			...
			final CommentResponse commentResponse1 = new CommentResponse(5L, 1L, "사과나무", memberImageURL, false, 33, 3,
				"이옷 정말 예뻐요~!", LocalDateTime.now(), LocalDateTime.now());
			final CommentResponse commentResponse2 = new CommentResponse(4L, 2L, "느티나무", memberImageURL, false, 31, 10,
				"강추 강추!", LocalDateTime.now(), LocalDateTime.now());
			final CommentResponse commentResponse3 = new CommentResponse(3L, 3L, "포도나무", memberImageURL, false, 22, 1,
				"좋네요", LocalDateTime.now(), LocalDateTime.now());

			final CommentPageResponse commentPageResponse = CommentPageResponse.of(true,
				List.of(commentResponse1, commentResponse2, commentResponse3));
	...
	class 투표_댓글_목록_전체_조회_테스트 {
		...
		void 투표_댓글_목록_전체_조회_성공() throws Exception {
			...
			final Long voteId = 1L;
			final String memberImageURL = "https://.../image.jpg";
			final CommentResponse commentResponse1 = new CommentResponse(5L, 1L, "사과나무", memberImageURL, false, 33, 1,
				"이옷 정말 예뻐요~!", LocalDateTime.now(), LocalDateTime.now());
			final CommentResponse commentResponse2 = new CommentResponse(4L, 2L, "느티나무", memberImageURL, false, 31, 10,
				"강추 강추!", LocalDateTime.now(), LocalDateTime.now());
			final CommentResponse commentResponse3 = new CommentResponse(3L, 3L, "포도나무", memberImageURL, false, 22, 24,
				"좋네요", LocalDateTime.now(), LocalDateTime.now());

			given(voteService.searchAllComments(any(), any())).willReturn(
				List.of(commentResponse1, commentResponse2, commentResponse3));
			...
			resultActions.andDo(restDocs.document(
				...
				responseFields(
					...
					subsectionWithPath("[].memberImageUrl").type(JsonFieldType.STRING)
	...
	class 투표_참여_회원_테스트 {
		@Test
		@DisplayName("투표 참여 회원 조회 API 테스트")
		void findVoteParticipants() throws Exception {

			final Long voteId = 10L;
			mockingForAuthorization();
			List<MemberVoteParticipantsDto> memberVoteParticipants = new ArrayList<>();
			memberVoteParticipants.add(new MemberVoteParticipantsDto(1L, "nickname1",
				"https://salmal-image.s3.ap-northeast-2.amazonaws.com/default.png"));
			memberVoteParticipants.add(new MemberVoteParticipantsDto(2L, "nickname2",
				"https://salmal-image.s3.ap-northeast-2.amazonaws.com/default.png"));
			memberVoteParticipants.add(new MemberVoteParticipantsDto(3L, "nickname3",
				"https://salmal-image.s3.ap-northeast-2.amazonaws.com/default.png"));

			VoteParticipantsResponse voteParticipantsResponse = new VoteParticipantsResponse(memberVoteParticipants);
			given(voteService.findVoteParticipants(anyLong())).willReturn(voteParticipantsResponse);

			// expect
			String URL = "/{vote-id}/participants";
			final ResultActions resultActions = mockMvc.perform(
					RestDocumentationRequestBuilders.get(BASE_URL + URL, voteId)
						.header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN)
						.characterEncoding(StandardCharsets.UTF_8))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.memberVoteParticipants[0].id").value(1L))
				.andExpect(jsonPath("$.memberVoteParticipants[0].nickname").value("nickname1"))
				.andExpect(jsonPath("$.memberVoteParticipants[0].profileImageUrl").value(
					"https://salmal-image.s3.ap-northeast-2.amazonaws.com/default.png"))

				.andExpect(jsonPath("$.memberVoteParticipants[1].id").value(2L))
				.andExpect(jsonPath("$.memberVoteParticipants[1].nickname").value("nickname2"))
				.andExpect(jsonPath("$.memberVoteParticipants[1].profileImageUrl").value(
					"https://salmal-image.s3.ap-northeast-2.amazonaws.com/default.png"))

				.andExpect(jsonPath("$.memberVoteParticipants[2].id").value(3L))
				.andExpect(jsonPath("$.memberVoteParticipants[2].nickname").value("nickname3"))
				.andExpect(jsonPath("$.memberVoteParticipants[2].profileImageUrl").value(
					"https://salmal-image.s3.ap-northeast-2.amazonaws.com/default.png"));
			...
			resultActions.andDo(restDocs.document(
				...
				pathParameters(
					parameterWithName("vote-id").description("조회할 투표 아이디")
				),
				responseFields(
					beneathPath("memberVoteParticipants").withSubsectionId("memberVoteParticipants"),
					subsectionWithPath("id").type(JsonFieldType.NUMBER).description("투표한 회원의 ID"),
					subsectionWithPath("nickname").type(JsonFieldType.STRING).description("투표한 회원의 닉네임"),
					subsectionWithPath("profileImageUrl").type(JsonFieldType.STRING).description("투표한 회원의 프로필 이미지")
				)
			));
