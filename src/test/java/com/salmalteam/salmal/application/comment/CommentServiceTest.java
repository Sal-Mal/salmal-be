...
class CommentServiceTest {
	...
	class 댓글_수정_테스트 {
		...
		void 요청_회원이_존재하지_않는다면_예외가_발생한다() {
			...
			// when & then
			assertThatThrownBy(() -> commentService.updateComment(authPayload, commentId, voteCommentUpdateRequest))
	...
	class 댓글_좋아요_테스트 {
		...
		void 좋아요할_댓글이_존재하지_않으면_예외가_발생한다() {
			...
			// when & then
			assertThatThrownBy(() -> commentService.likeComment(authPayload, commentId))
				.isInstanceOf(CommentException.class);
	...
	class 댓글_좋아요_취소_테스트 {
		...
		void 좋아요_취소할_댓글이_존재하지_않으면_예외가_발생한다() {
			...
			// when & then
			assertThatThrownBy(() -> commentService.unLikeComment(authPayload, commentId))
				.isInstanceOf(CommentException.class);
	...
	class 댓글_신고_테스트 {
		...
		void 신고할_댓글이_존재하지_않을_경우_예외가_발생한다() {
			...
			// when & then
			assertThatThrownBy(() -> commentService.report(authPayload, commentId))
				.isInstanceOf(CommentException.class);
